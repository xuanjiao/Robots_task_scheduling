...
-- 	
   -- select pos,time_left,time_right;
	...
	update open_possibility_table  -- update possibility table with new posibility
		set statistic_door_open_posibility = (
			  select 100*sum(door_status)/count(door_status)  -- calculate possibility
						 from door_status_list
			where room = room_id and dayofweek(t) = dayofweek(date_time) and (TIME(date_time) between start_time and end_time)
        )
	where  room = room_id and dayofweek(t) = day_of_week and (time(t) between start_time and end_time);
